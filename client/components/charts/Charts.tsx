...
export function BarChart({ data }: { data: { name: string; income: number; expenses: number }[] }) {
  ...
  return (
    <div className="h-80">
      <ResponsiveContainer width="100%" height="100%">
        <RBarChart data={data} margin={{ top: 8, right: 16, left: 28, bottom: 8 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="name" tick={{ fill: axisColor, fontSize: 12 }} tickMargin={8} axisLine={{ stroke: axisColor }} />
          <YAxis tick={{ fill: axisColor, fontSize: 12 }} tickFormatter={formatINR} tickMargin={8} axisLine={{ stroke: axisColor }} />
          <RPTooltip formatter={(v: any) => [typeof v === 'number' ? `₹${v.toLocaleString()}` : v, '']} />
          <Legend />
