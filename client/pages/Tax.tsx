  ...
  return (
    <AppLayout>
      <div className="max-w-2xl mx-auto text-center">
        ...
        <div className="mt-6">
          <button className="px-5 py-3 rounded-lg bg-primary text-primary-foreground" onClick={() => exportFinanceToExcel({ incomes: incomes.map(i=>({ date: i.date, source: i.source, amount: i.amount, invoiceUrl: i.invoiceUrl })), expenses: expenses.map(e=>({ date: e.date, category: e.category, amount: e.amount, receiptUrl: e.receiptUrl })), reminders: reminders.map(r=>({ title: r.title, dueDate: r.dueDate, amount: r.amount })), savings: savings.map(s=>({ name: s.name, amount: s.amount, date: s.date })), variant })}>Download {variant} Excel</button>
        </div>
      </div>
    </AppLayout>
  );
