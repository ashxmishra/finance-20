...
export function exportFinanceToExcel(opts: {
  ...
  if (reminders.length)
    XLSX.utils.book_append_sheet(
      wb,
      XLSX.utils.json_to_sheet(reminders),
      "Reminders",
    );
  if (savings.length)
    XLSX.utils.book_append_sheet(
  ...
  const summary = [
    ...
    {
      field: "Total Incomes",
      value: incomes.reduce((a, b) => a + (b.amount || 0), 0),
    },
    {
      field: "Total Expenses",
      value: expenses.reduce((a, b) => a + (b.amount || 0), 0),
    },
    {
      field: "Savings",
      value:
        incomes.reduce((a, b) => a + (b.amount || 0), 0) -
  ...
  XLSX.writeFile(
    wb,
    `fintrack-${variant.toLowerCase()}-${new Date().toISOString().slice(0, 10)}.xlsx`,
  );
}
